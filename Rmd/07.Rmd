...
This part figures show cellular composition of engrafted cells following HSC transplantation over time.

### Set up data and load the environment.
```{r}
rm(list=ls())
pwd <- getwd()
...
### Set the cell type coordinate and add cell type label
...
### Plot the homeostasis and transplantation cells
```{r}
library(ggplot2)
...
for( i in time.order){
     ...
     plot_list[[i]] <- ggplot(data = tsne.df, aes(x = x,y = y)) + 
         geom_point(fill = "grey", shape = 21, size = 1.5, colour = "white", alpha = I(0.3)) +
         geom_point(data = dat, aes(fill = factor(New.Celltype)), shape = 21, colour = "white",size = 4) + 
         theme_classic() +
         scale_fill_manual(name = "Cell type", values = col.values) + 
         scale_alpha(guide = F)+ theme(text = element_text(size=8)) + 
         xlab("tSNE1") + ylab("tSNE2") +
         ggtitle(i)}
        # ggsave(filename = paste0(pwd,"/output/Figure.3e.Predict_cell_types.tsne.", i, ".pdf"), width=8)
ggpubr::ggarrange( plot_list[[1]],plot_list[[2]],plot_list[[3]],plot_list[[4]],plot_list[[5]], nrow = 2,ncol = 3)
```

### Figure 3f for dynamic frequencies of each cell type at day 1, 3, 5 and 7 after 

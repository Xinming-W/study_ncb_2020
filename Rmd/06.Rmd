...
<font color="red">Waring: the version of Monocle in the paper is Monocle2, but Here we use Monocle3 to get the cell trajectory.</font>

<strong>Note: cell subset of tMPPs were renumber that cell cluster 1, 2, 3, 4 and 5 represent tMPP5, tMPP4, tMPP1, tMPP2 and tMPP3 respectively(for more information please see the 04 step).</strong>
...
library(ggplot2)
library(plyr)
colnames(percent.data)=c("phenotype","predict_cell_type","counts","percent")
...
#### Plot the raw result(every plot contains one cell types).
```{r}
...
<font color="red">Waring: 
these codes just rotate the picture.Nothing else. 

If your figure' direction is as same as the figure in the paper ,don't use these codes.</font>
```{r}
...
#### Some other preparatory works for plot
```{r}
...
<font color="red">There are something wrong when using Knit to show the result in rMarkdown format.But it's correct in the R format. So we just show the result saving with runing r code in Rstudio.</font>
...
![](D:/lab/HSC/data/Codes_for_Fang_Dong_et_al_NCB_2020-master/output/Monocle_per_celltype.jpeg)
...
![](D:/lab/HSC/data/Codes_for_Fang_Dong_et_al_NCB_2020-master/output/Monocle_all_celltype.jpeg)
